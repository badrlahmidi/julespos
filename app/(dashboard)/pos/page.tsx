...
export default function PosPage() {
  ...
  useEffect(() => {
    // Auto-open a table/order on mount for demo purposes
    if (!currentOrder) {
      openTable('table-1');
    }
  }, [currentOrder, openTable]);
  ...
  return (
    <div className="flex h-screen w-full bg-pos-dark text-pos-text-primary overflow-hidden">
      ...
      <aside className="w-[80px] bg-pos-darker border-r border-pos-card flex flex-col items-center py-6 shrink-0">
        <div className="w-10 h-10 bg-pos-emerald rounded-pos flex items-center justify-center mb-8 shadow-md">
          <span className="font-bold text-white text-xl">R</span>
        </div>

        <nav className="flex flex-col gap-6 flex-1">
          <NavItem icon={<LayoutGrid size={24} />} isActive={false} />
          <NavItem icon={<Zap size={24} />} isActive={true} />
      ...
      <OrderTicket tableName="Vente au comptoir" />
...
function NavItem({ icon, isActive, className = "" }: { icon: React.ReactNode, isActive: boolean, className?: string }) {
  return (
    <button
